...
class PascalVOC2coco(object):
    ...
    def data_transfer(self):
        for num, json_file in enumerate(self.xml):
            ...
            self.num = num
            im_file = json_file.replace('.xml', '.jpg').replace('box', 'image')
            print(im_file)
            self.filename = os.path.basename(im_file)
            self.height, self.width, _ = cv2.imread(im_file).shape
...
path = r'D:\2-deep_learning\0-data\undersea\train\box\\'
